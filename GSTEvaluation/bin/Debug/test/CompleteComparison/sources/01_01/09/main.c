	 ...
	 do
        {
                ...
                scanf("%d",&deg);
					...
					sin100vorkomma = abs (Sin100(DegToRad(deg))/100);
                    sin100nachkomma = abs (Sin100(DegToRad(deg))%100);

					if (deg >= 0) {

					printf("Der Winkel %d\xF8 betraegt umgerechnet %d.%02d rad \n", deg, cradvorkomma,
						cradnachkomma);
					printf("Der Sinus dieses rad-Wertes ist: %d.%02d \n",
...
int Sin100 (int rad)
{
	int sin100;
				...
				sin100 = rad - ((rad*rad*rad)/(60000)) +((rad*rad*rad)/(60000)*(rad*rad)/(200000)) - ((rad*rad*rad)/(60000)*(rad*rad)/(200000) * (rad*rad)/(420000));
